...
function loadControl(month, year) {
    ...
    for (let rowIterator = 0; rowIterator < 6; rowIterator++) {
        ...
        for (let cellIterated = 0; cellIterated < 7 && date <= daysInMonth(month, year); cellIterated++) {
            ...
            if (rowIterator !== 0 || cellIterated >= firstDay) {
                ...
                //  if( new Date(cell.id)<today){
                //     cell.classList.add('old_date');
                // }
                ...
                for(i=0; i<dsss.length; i++){

                    if(cell.id == dsss[i]){

                        cell.classList.add('able_able');
    ...
    $("#calendarBody tr td").click(function (e) {
       ...
       // End try section
        ...
        var id = $(this).attr('id');
        var select_cal_date = new Date(id);
        ...
        /*booking day count section */



        /*end booking day count section */
    ...
    for(i=0; i<dsss.length; i++){
        ...
        if (typeof id !== typeof undefined) {
            ...
            var select_day_booking = selectedDates;
            $('.userSelectDayCount').val(select_day_booking.length);
